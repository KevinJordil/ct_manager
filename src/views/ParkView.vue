...
<template>
  <div class="flex flex-col gap-4">
    ...
    <div v-if="!store.imageUrl && store.loaded"
      class="border-2 border-dashed border-gray-300 rounded-2xl bg-gray-50 flex flex-col items-center justify-center py-20 gap-4 transition-colors hover:border-blue-400 hover:bg-blue-50/30"
      @dragover.prevent @drop.prevent="onDrop">
      <svg class="w-14 h-14 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
          d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>
      </svg>
      <div class="text-center">
        <p class="text-gray-700 font-medium">{{ $t('park.noImage') }}</p>
        <p class="text-sm text-gray-500 mt-1">{{ $t('park.dropHint') }}</p>
      </div>
    ...
    <div v-else-if="store.imageUrl" class="flex flex-col gap-3">
      ...
      <p v-if="imageWidth && imageHeight" class="text-xs text-gray-400">
      ...
      <div class="flex flex-col items-center gap-2">
        <div v-if="!editing && store.zones.length"
          class="flex flex-wrap justify-center gap-4 text-xs text-gray-600">
      ...
      <Transition name="fade">
        <div v-if="editing && selectedZone" class="card border-gray-200">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label class="label" for="zone-label">
                {{ $t('park.label') }}
                <span class="ml-1 font-normal text-gray-400">{{ $t('park.labelHint') }}</span>
              </label>
      ...
      <Transition name="fade">
        <div v-if="editing && !selectedZone"
          class="flex flex-wrap items-center gap-3 px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm">
          <span class="text-gray-600 font-medium shrink-0">{{ $t('park.nextZoneColor') }}</span>
