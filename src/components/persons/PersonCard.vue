...
import { getPersonStatutAffiche, missionActuelleDePersonne } from '../../availability.js'
...
const missionEnCours = computed(() =>
  missionActuelleDePersonne(props.person.id, missionsStore.missions, nowStr.value)
)

const statut = computed(() =>
  getPersonStatutAffiche(props.person, missionsStore.missions, nowStr.value)
)

const congesActifs = computed(() =>
  (props.person.conges ?? []).filter(c => c.dateFin >= nowStr.value)
    ...
    <div class="flex items-start justify-between gap-2">
      ...
      <div class="flex gap-1 shrink-0">
        <button @click="$emit('toggle-indisponible')"
          ...
          :aria-label="person.indisponible ? 'Lever l\'indisponibilité' : 'Marquer indisponible'">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/>
          </svg>
        </button>
        <button @click="$emit('manage-conges')" class="icon-btn" title="Gérer les congés" aria-label="Gérer les congés">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
          </svg>
        </button>
        <button @click="$emit('edit')" aria-label="Modifier la personne" title="Modifier" class="icon-btn">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
          </svg>
        </button>
        <button @click="$emit('delete')" aria-label="Supprimer la personne" title="Supprimer" class="icon-btn text-red-400 hover:text-red-600">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
