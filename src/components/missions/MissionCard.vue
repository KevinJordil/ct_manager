    ...
    <div class="flex items-start justify-between gap-2">
      <div class="flex-1 min-w-0">
        ...
        <div v-if="assignedVehicles.length" class="mt-2 space-y-1">
          <div v-for="entry in assignedVehicles" :key="entry.id" class="flex items-center gap-1.5 text-sm text-gray-600">
            ...
            <span class="font-medium">{{ entry.vehicle?.name ?? $t('common.empty') }}</span>
            ...
            <span v-else class="text-xs text-gray-400 italic">{{ $t('missions.noDriver') }}</span>
      ...
      <div class="flex gap-1 shrink-0">
        <button @click="$emit('print')" :aria-label="$t('printing.printMission')"
          :title="$t('printing.printMission')" class="icon-btn">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/>
          </svg>
        </button>
        <button @click="$emit('edit')" :aria-label="$t('missions.edit')" :title="$t('actions.edit')" class="icon-btn">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
          </svg>
        </button>
        <button @click="$emit('delete')" :aria-label="$t('missions.deleteTitle')" :title="$t('actions.delete')" class="icon-btn text-red-400 hover:text-red-600">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
